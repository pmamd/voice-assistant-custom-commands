...
{
	// FIX: Early return if text is empty
	if (text.empty()) return;

	// remove (text) and <tag> using regex
	// FIX: Safe parenthesis handling with size check
	if (text.size() >= 1) {
	...
	// FIX: Check if text became empty after regex replacements
	if (text.empty()) return;
	...
	// FIX: Safe index access with size checks
	else if (!text.empty() && text[text.size()-1] == ':') text[text.size()-1] = ' ';
	else if (!text.empty() && text[text.size()-1] == '-') text[text.size()-1] = ' ';
	else if (!text.empty() && text[text.size()-1] == '(') text[text.size()-1] = ' ';
	else if (!text.empty() && text[text.size()-1] == ',') text[text.size()-1] = ' ';
	else if (!text.empty() && text[text.size()-1] == '.') text[text.size()-1] = ' ';
	...
	// FIX: Check if text became empty after the above operations
	if (text.empty()) return;

	// FIX: Safe access - check size BEFORE accessing index
	...
	// FIX: Safe .back() usage - check if not empty first
	if (!text.empty() && text.back() == ':' && text.length() < 15 && text.find(' ') == std::string::npos) text = "";

	// Final validation and CURL request
	if (text.size()>=1 && text != "." && text != "," && text != "!" && text != "\n")
	{
		...
		// FIX: Check if curl_easy_init() succeeded
		...
		// FIX: Store header list so we can free it later
		...
		// FIX: Check if curl_multi_init() succeeded
		CURLM *multi_handle = curl_multi_init();
		if (!multi_handle) {
			fprintf(stderr, "[TTS] ERROR: Failed to initialize CURL multi handle\n");
		...
		// FIX: Proper cleanup including header list
		curl_slist_free_all(headers);
		curl_easy_cleanup(http_handle);
...
int run(int argc, const char ** argv) {
    ...
    while (is_running) {
		...
		// audio
        {
			...
			bool vad_result = ::vad_simple(pcmf32_cur, WHISPER_SAMPLE_RATE, params.vad_last_ms, params.vad_thold, params.freq_thold, params.print_energy);			
